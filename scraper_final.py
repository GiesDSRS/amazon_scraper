...
def main():
    ...
    #driver = webdriver.Chrome(executable_path='/Users/akashrai/Desktop/chromedriver')
    ...
    #search = "Foam Roller"
    #URL = "https://www.amazon.com/s?k=foam+roller"
    search = input("Enter search value:")
    URL = "https://www.amazon.com/s?k=" + generate_search_for_URL(search)
    next = URL
    ...
    while len(URL) != 0:
        ...
        for items in results:
            ...
            ranking_url = "https://www.amazon.com" + get_ranking(url_product)
            if ranking_url[44:52] == "Redirect":
                continue;
            ...
            if soup2.find('div' , {'id' : 'dpx-detail-bullets_feature_div'}) != None:
                ranking = soup2.find('div' , {'id' : 'dpx-detail-bullets_feature_div'})
            ...
            elif soup2.find('div' , {'id' : 'detailBulletsWrapper_feature_div'}) != None:
                ...
                if len(ranking2) >=2:
                    ranking3 = str(ranking2[1])
                    get_category_ranking(ranking3, search)
            ...
            elif soup2.find('ul', {'class' : 'a-unordered-list a-nostyle a-vertical a-spacing-none detail-bullet-list'}) != None:
                ranking2 = soup2.find_all('ul', {'class' : 'a-unordered-list a-nostyle a-vertical a-spacing-none detail-bullet-list'})
...
def get_category_ranking(ranking3, search):
    ...
    if (' '+search+' ') in (' ' + ranking3 + ' ') == false :
        return -1

    c = "#"
    foo = ( [pos for pos, char in enumerate(ranking3) if char == c])

    if len(foo) == 0:
        return -1

    pos = ranking3.find(search)
    pos_of_ranking = 0
